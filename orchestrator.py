...
def RunCampaign():
    ...
    InitializeExperimentPlan(experimentPlanStoragePath=f"namespace/{collectionName}/experiment_plan")
    PrepareDrop()
    CaptureImage(experimentPlanPath=f"namespace/{collectionName}/experiment_plan", imageStoragePath=f"namespace/{collectionName}/image")
    AnalyseImage(imagePath=f"namespace/{collectionName}/image", experimentPlanPath=f"namespace/{collectionName}/experiment_plan", analysisStoragePath=f"namespace/{collectionName}/analysis")
    ...
    while True:
        ...
        collectionName = f"collection_{experimentIndex}"
        dataClient.DataItemProvider.CreateDataCollection(CollectionPath=f"namespace/{collectionName}")
        PlanExperiment(previousAnalysisPath=f"namespace/collection_{experimentIndex-1}/analysis", experimentPlanStoragePath=f"namespace/{collectionName}/experiment_plan")
        experimentPlan = json.loads(dataClient.DataItemProvider.GetDataItem(ItemPath=f"namespace/{collectionName}/experiment_plan").DataItemContent.decode('utf-8'))
...
def AnalyseImage(imagePath, experimentPlanPath, analysisStoragePath):
    analyserClient.PendantDropAnalyserController.AnalyseImage(ImagePath=imagePath, ExperimentPlanPath=experimentPlanPath)
    analyserClient.PendantDropAnalyserController.StoreAnalysisResulsts(ItemPath=analysisStoragePath)

@task
def PlanExperiment(previousAnalysisPath, experimentPlanStoragePath):
    plannerClient.BinarySearchController.UpdateSearchRange(MeasurementPath=previousAnalysisPath)
    plannerClient.BinarySearchController.CalculateMidPoint()
    plannerClient.ExperimentPlanProvider.CreateExperimentPlan(PreviousMeasurementItemPath=previousAnalysisPath)
    plannerClient.ExperimentPlanProvider.StoreExperimentPlan(ItemPath=experimentPlanStoragePath)
...
def InitializeExperimentPlan(experimentPlanStoragePath):
    ...
    plannerClient.BinarySearchController.CalculateMidPoint()
    plannerClient.ExperimentPlanProvider.SubmitExperimentDesignPlan(Target=42, Tolerance=0.35)
    plannerClient.ExperimentPlanProvider.StoreExperimentPlan(ItemPath=experimentPlanStoragePath)
